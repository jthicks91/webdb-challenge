...
1. Explain the difference between `RDBMS` and `SQL`.
  -SQL is a programming language that is used to communicate with data sets and a RDBMS stands for a relational database management system which is the entire system for managing storage adn relationships between specific data sets.

1. Why do tables need a `primary key`?
  Primary keys provide unique and distinct identifiers for each of the records for maintenance and usage;

1. What is the name given to a table column that references the primary key on another table.
  - Foreign Key

1. What do we need in order to have a _many to many_ relationship between two tables.
  a third table relating Primary keys and foreign keys on teh related tables;
